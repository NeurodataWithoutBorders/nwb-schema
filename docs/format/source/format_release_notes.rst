...
- Changed ``NWBFile.electrodes`` from a generic ``DynamicTable`` with added columns to a new ``ElectrodesTable`` neurodata type that extends ``DynamicTable`` with added columns. (#539, #624)
